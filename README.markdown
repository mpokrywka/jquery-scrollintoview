...
This fork is npm, bower, AMD and jquery 1.8 compatibale which includes changes made by [Michael Best](https://github.com/mbest), [Michael Reinsch](https://github.com/mreinsch) and [Adam Tal](https://github.com/adamtal3).
...
* **viewPadding** (default: 0) - when positioning the element to scroll into view, this value (in pixels) is applied to the outside of the element so that some space is visible around the element. This can be a single number that applies to both directions, or it can be an object with **x** and/or **y** values or **left**/**right**/**top**/**bottom** for custom padding for each side.
